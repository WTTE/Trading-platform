<template>
  <div>
    ...
    <el-card class="box-card">
      ...
      <span>数据列表</span>
      <router-link to="/permission/newAdmin">
        <el-button
          size="mini"
          class="btn-add"
    ...
    <div class="box-card">
      ...
      >
        ...
        <el-table-column label="管理员姓名" width="100" align="center">
          <template slot-scope="scope">{{ scope.row.name }}</template>
        </el-table-column>
        <el-table-column label="管理员编号" width="160" align="center">
          <template slot-scope="scope">{{ scope.row.workNum }}</template>
        </el-table-column>
        <el-table-column label="管理员等级" width="110" align="center">
    ...
    handleDialogConfirm() {
      this.$confirm("是否要确认?", "提示", {
        ...
        if (this.isEdit) {
          ...
          addAdmin(this.list).then((response) => {
            this.$message({
              message: "添加成功！",
              type: "success",
            });
